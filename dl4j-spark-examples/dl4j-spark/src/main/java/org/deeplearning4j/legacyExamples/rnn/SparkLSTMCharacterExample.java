package org.deeplearning4j.legacyExamples.rnn;
 ...
 * (for example, multi-GPU systems) instead use ParallelWrapper (which is faster than using Spark for training on a
 * single machine).
 * See for example MultiGpuLenetMnistExample in dl4j-cuda-specific-examples
 * <p>
 * To run the example using Spark submit (for example on a cluster): pass "-useSparkLocal false" as the application
 * argument,
...
public class SparkLSTMCharacterExample {
	...
	@Parameter(names = "-useSparkLocal", description = "Use spark local (helper for testing/running without spark " +
			"submit)", arity = 1)
	private boolean useSparkLocal = false;

	@Parameter(names = "-batchSizePerWorker", description = "Number of examples to fit each worker with")
	private int batchSizePerWorker = 400;   //How many examples should be used per worker (executor) when fitting?
	...
	protected void entryPoint(String[] args) throws Exception {
		...
		int tbpttLength = 50;                       //Length for truncated backpropagation through time. i.e., do
		// parameter updates ever 50 characters
		int nSamplesToGenerate = 4;                    //Number of samples to generate after each training epoch
		int nCharactersToSample = 300;                //Length of each sample to generate
		String generationInitialization = null;        //Optional character initialization; a random character is used
		// if null
		...
		MultiLayerConfiguration conf = new NeuralNetConfiguration.Builder().seed(12345)
				...
				.list()
				.layer(0,
						new LSTM.Builder().nIn(CHAR_TO_INT.size())
								.nOut(lstmLayerSize)
								.activation(Activation.TANH)
								.build())
				.layer(1,
						new LSTM.Builder().nIn(lstmLayerSize).nOut(lstmLayerSize).activation(Activation.TANH).build())
				.layer(2,
						new RnnOutputLayer.Builder(LossFunction.MCXENT).activation(Activation.SOFTMAX)        //MCXENT
		...
		ParameterAveragingTrainingMaster tm =
				new ParameterAveragingTrainingMaster.Builder(examplesPerDataSetObject).workerPrefetchNumBatches(
				2)    //Asynchronously prefetch up to 2 batches
				.averagingFrequency(averagingFrequency).batchSizePerWorker(batchSizePerWorker).build();
		...
		for (int i = 0; i < numEpochs; i++) {
			log.info("Started training epoch {} of {}", i, numEpochs);
...
/*            //Sample some characters from the network (done locally)
			...
			log.info("Finished training epoch {} of {}", i, numEpochs);
		}
	...
	public static JavaRDD<DataSet> getTrainingData(JavaSparkContext sc) throws IOException {
		...
		// File -> String -> List<String> (split into length "sequenceLength" characters) -> JavaRDD<String> ->
		// JavaRDD<DataSet>
	 ...
	 *
	 * @param initialization     String, may be null. If null, select a random character as initialization for all
	 *                              samples
	...
	private static String[] sampleCharactersFromNetwork(String initialization, MultiLayerNetwork net, Random rng,
		...
		for (int i = 0; i < charactersToSample; i++) {
			...
			for (int s = 0; s < numSamples; s++) {
				...
				sb[s].append(intToChar.get(sampledCharacterIdx));    //Add sampled character to StringBuilder (human
				// readable output)
			}
